...
. In the _description_ of your _Substrate_ PR add "polkadot companion: [AXIA_PR_URL]"
. Now you should see that the `check_polkadot` CI job will build your Substrate PR agains the mentioned AXIA branch in your PR description.
. Someone will need to approve the AXIA PR before the Substrate CI will go green. (The AXIA CI failing can be ignored as long as the polkadot job in the _substrate_ PR is green).
...
If your PR is reviewed well, but a AXIA PR is missing, signal it with https://github.com/paritytech/substrate/labels/A7-needspolkadotpr[`A7-needspolkadotpr`] to prevent it from getting automatically merged.
